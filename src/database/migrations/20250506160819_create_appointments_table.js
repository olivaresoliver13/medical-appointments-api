export async function up(knex) {
  return knex.schema.createTable('appointments', (table) => {
    ...
    table.integer('doctor_id').unsigned().references('id').inTable('doctors').onDelete('CASCADE').notNullable();
    table.dateTime('appointment_time').notNullable();
    table.enum('status', ['pendiente', 'confirmada', 'rechazada', 'cancelada']).defaultTo('pendiente');
